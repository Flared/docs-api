...
The `ApiClient` has `IterGet` and `IterPostJson` methods that return an iterator that implements the
[Flare standard paging pattern <Icon icon="book" size={16} />](/concepts/paging).

The paging utils leverage Go iterators which require Go version >= 1.23.
