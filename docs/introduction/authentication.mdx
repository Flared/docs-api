...
# Use request.Session so that we remember cookies.
...
# Now we can use our cookies to access the testing endpoint.
resp = session.get("https://api.flare.io/tokens/test")
print(resp.json())
```
</Accordion>
